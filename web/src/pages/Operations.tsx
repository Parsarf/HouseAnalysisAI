...
export function BatchesPage() {
  ...
  return <section><PageHeader eyebrow="Ingestion" title="Batches" description="Bring source documents into ACQ with a deliberate cost confirmation before extraction." />{error && <div className="inline-error">{error}</div>}<div className="batch-layout"><section className="panel upload-panel"><div className="panel-heading"><div><span className="eyebrow">Source documents</span><h2>Upload property reports</h2></div></div><input className="text-input" placeholder="Batch name (optional)" value={name} onChange={(e) => setName(e.target.value)} disabled={user.read_only} /><button type="button" className={`drop-zone ${dragging ? "dragging" : ""}`} disabled={user.read_only} onClick={() => fileRef.current?.click()} onDragOver={(e) => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={(e) => { e.preventDefault(); setDragging(false); acceptFiles([...e.dataTransfer.files]); }}><span className="upload-icon">↑</span><strong>Drop PDF reports here</strong><small>or choose files from your computer</small></button><input ref={fileRef} hidden multiple type="file" accept="application/pdf,.pdf" onChange={(e) => acceptFiles([...(e.target.files ?? [])])} />{files.length > 0 && <div className="file-list">{files.map((file) => <div key={`${file.name}-${file.size}`}><span>{file.name}</span><small>{(file.size / 1_000_000).toFixed(1)} MB</small></div>)}</div>}<button className="btn btn-primary btn-block" disabled={!files.length || busy || user.read_only} onClick={upload}>{busy ? "Uploading…" : `Upload ${files.length || ""} report${files.length === 1 ? "" : "s"}`}</button><div className="or-divider"><span>or paste text</span></div><textarea className="text-area" rows={6} value={paste} onChange={(e) => setPaste(e.target.value)} placeholder="Paste a property report or source text…" disabled={user.read_only} /><button className="btn btn-secondary btn-block" disabled={!paste.trim() || busy || user.read_only} onClick={ingest}>Create batch from text</button></section><section className="panel run-panel"><div className="panel-heading"><div><span className="eyebrow">Spend gate</span><h2>{batch ? `Batch ${batch.name ?? batch.id.slice(0,8)}` : "No active batch"}</h2></div>{batch && <span className={`status-pill status-${batch.status}`}>{batch.status.replace(/_/g, " ")}</span>}</div>{!batch ? <div className="empty-state"><strong>Upload reports to begin</strong><span>ACQ will inspect the batch and show estimated model usage before any paid extraction begins.</span></div> : <><div className="progress-summary"><div><span>Progress</span><strong>{batch.completed + batch.failed} / {batch.total}</strong></div><div className="progress-track"><span style={{ width: `${progress}%` }} /></div><div className="progress-legend"><span>{batch.completed} complete</span><span>{batch.failed} failed</span><span>{progress}%</span></div></div>{batch.status === "uploaded" && !estimate && <div className="callout"><strong>Ready for pre-flight estimate</strong><p>This calculates tokens and cost only. Extraction will remain paused.</p><button className="btn btn-primary" disabled={busy} onClick={estimateNow}>Calculate estimate</button></div>}{estimate && <div className="estimate-card"><div><span>Reports</span><strong>{estimate.report_count}</strong></div><div><span>Estimated tokens</span><strong>{estimate.total_tokens.toLocaleString()}</strong></div><div><span>Estimated cost</span><strong><MoneyText money={money(estimate.estimated_cost_usd, true)} /></strong></div><p>Nothing proceeds until you confirm this spend.</p><button className="btn btn-primary btn-block" disabled={busy || user.read_only} onClick={start}>Start extraction</button></div>}{batch.status === "awaiting_confirmation" && !estimate && <div className="callout warning"><strong>Awaiting confirmation</strong><p>This batch is paused at the spend gate.</p><button className="btn btn-primary" onClick={estimateNow}>Review estimate</button></div>}{batch.status === "paused_budget" && <div className="callout danger"><strong>Paused by budget limit</strong><p>Increase the configured budget before resuming extraction.</p></div>}{batch.results && batch.results.length > 0 && <div className="callout batch-results"><strong>Analysis complete</strong><p>Your property analysis is ready.</p>{batch.results.map((result) => <div className="batch-result" key={result.property_id}><span>{batchResultLabel(result)}</span><Link className="btn btn-primary" to={`/properties/${result.property_id}`}>View analysis</Link></div>)}</div>}{batch.unresolved_reports && batch.unresolved_reports.length > 0 && <div className="callout danger"><strong>Property identity could not be resolved</strong><p>The extracted evidence was preserved, but it did not contain a grounded street address. No property was fabricated.</p><Link to="/problems">Review unresolved reports →</Link></div>}{["running","complete","completed","failed"].includes(batch.status) && <div className="cost-row"><div><span>Estimated</span><MoneyText money={money(batch.estimated_cost_usd, true)} /></div><div><span>Actual</span><MoneyText money={money(batch.actual_cost_usd)} /></div>{batch.failed > 0 && <Link to="/problems">Review failures →</Link>}</div>}</>}</section></div></section>;
}
