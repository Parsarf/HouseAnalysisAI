...
export function PropertiesPage() {
  ...
  const [undoArchive, setUndoArchive] = useState<PropertyListItem | null>(null);

  const load = useCallback(async () => {
    setLoading(true); setError(null);
    try { const response = await listProperties({ sort, order, filters: clauses, limit: 50, showArchived }); setItems(response.items); setNextCursor(response.next_cursor); setSelected(new Set()); }
    catch (reason) { setError(reason instanceof Error ? reason : new Error("Failed to load portfolio")); }
    finally { setLoading(false); }
  }, [clauses, order, showArchived, sort]);
  ...
  const loadMore = async () => { if (!nextCursor) return; setLoadingMore(true); try { const response = await listProperties({ sort, order, filters: clauses, cursor: nextCursor, limit: 50, showArchived }); setItems((value) => [...value, ...response.items]); setNextCursor(response.next_cursor); } catch (reason) { setError(reason instanceof Error ? reason : new Error("Unable to load more")); } finally { setLoadingMore(false); } };
  ...
  const archive = async (item: PropertyListItem) => {
    if (!window.confirm(`Archive ${addressOf(item)}?`)) return;
    await archiveProperty(item.id); setItems((current) => current.filter((row) => row.id !== item.id)); setUndoArchive(item);
  };
  const undo = async () => { if (!undoArchive) return; await restoreProperty(undoArchive.id); setItems((current) => [undoArchive, ...current]); setUndoArchive(null); };
